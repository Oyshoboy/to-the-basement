...
namespace RayFire
{
    ...
    public class RFPhysic
    {
        ...
        //[Tooltip(".")]
        [HideInInspector]public bool ignoreNeighbour = false;
        //[Space (2)]
        ...
        public static void SetFragmentMeshCollider(RayfireRigid scr, Mesh mesh)
        {
            ...
            // Skip collider
            SetCollider (scr, mesh);
        }
        
        // Set fragments collider
        public static void SetCollider (RayfireRigid scr, Mesh mesh = null)
        {
            // Skip collider
        ...
        public static void SetCollider (RayfireRigidRoot scr, RFShard shard)
        {
            ...
            if (shard.col == null)
            {
                ...
                // Box / Sphere collider
                    ...
                    shard.col = scr.gameObject.AddComponent<BoxCollider>();
                else if (scr.physics.colliderType == RFColliderType.Sphere)
                    shard.col = scr.gameObject.AddComponent<SphereCollider>();
            }
            
            // TODO check if meshCollider without mesh
        }
        ...
        public static void SetIgnoreColliders(RFPhysic physics, List<RFShard> shards)
        {
            if (physics.ignoreNeighbour == true)
            {
                ...
                if (physics.HasIgnore == false)
                    physics.ignoreList = GetIgnoreList(shards);
        ...
        public static List<int> GetIgnoreList(List<RFShard> shards)
        {
            ...
            {
                for (int n = 0; n < shards.Count; n++)
                {
                    if (s != n)
                    {
                        if (shards[s].bnd.Intersects (shards[n].bnd) == true)
                        {
                            pruneList.Add (shards[s].id);
                            pruneList.Add (shards[n].id);
                        }
                    }
                }
            }
