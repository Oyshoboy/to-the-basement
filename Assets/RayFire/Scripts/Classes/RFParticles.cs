    ...
    public class RFParticles
    {
        ...
        public static void SetParticleComponents(RayfireRigidRoot scr)
        {
            ...
            for (int i = 0; i < scr.cluster.shards.Count; i++)
                scr.sizeSum += scr.cluster.shards[i].sz;
        ...
        public static void InitActivationParticlesShard(RayfireRigidRoot source, RFShard shard)
        {
            ...
            if (source.HasDebris == true)
                for (int i = 0; i < source.debrisList.Count; i++)
                    if (source.debrisList[i].onActivation == true)
                        CreateDebrisShard (source, source.debrisList[i], shard);
            
            // Create dust particles
            if (source.HasDust == true)
                for (int i = 0; i < source.dustList.Count; i++)
                    if (source.dustList[i].onActivation == true)
                        CreateDustShard (source, source.dustList[i], shard);
        }
        ...
        {
            // Set amount
            target.amountFinal = GetShardFinalAmount (shard, target.emission.burstType, target.emission.burstAmount, root.sizeSum);
            ...
            // Particle system
            CreateParticleSystemDebris (target, shard.tm);
            
            // Get emit material index
            int emitMatIndex = GetEmissionMatIndex (shard.tm.GetComponent<Renderer>(), target.emissionMaterial);

            // Create debris
            target.CreateDebris (target, shard.mf, emitMatIndex, target.pSystem);
        }
        ...
        public static void CreateDustShard(RayfireRigidRoot root, RayfireDust target, RFShard shard)
        {
            ...
            // Particle system
            CreateParticleSystemDust (target, shard.tm);

            // Get emit material index
            int emitMatIndex = GetEmissionMatIndex (shard.tm.GetComponent<Renderer>(), target.emissionMaterial);

            // Create debris
            target.CreateDust (target, shard.mf, emitMatIndex, target.pSystem);
        }
